...
@section('content')
  <main class="p-6" style="font-family: var(--font-secondary);">
    <div class="flex items-start justify-between mb-6">
      <div>
        <h1 class="text-3xl font-extrabold mb-1" style="font-family: var(--font-primary); color: var(--text-title);">Clientes</h1>
        <p class="text-sm text-gray-600">Gestiona la información de los clientes</p>
      </div>

      <div class="ml-4">
  <button type="button" class="inline-flex items-center px-4 py-2 rounded-md text-white btn-green js-open-form-modal" data-modal-target="#clientModal" style="background: var(--green);">
    ...
    <div class="mb-6">
      ...
      <div class="relative">
        ...
        <input id="search" name="search" type="search" placeholder="Buscar por nombre del cliente" class="w-full pl-12 pr-4 py-3 rounded-lg border" style="background: var(--bg-input); border-color: var(--border);">
      </div>
    </div>
...
@component('components.modal-form', ['id' => 'clientModal', 'title' => 'Agregar Nuevo Cliente'])
  <form method="POST" action="#" class="space-y-4">
    @csrf
    @include('components.forms.client-form')
    <div class="mt-4 flex items-center justify-end gap-3">
        <button type="button" class="px-5 py-2 rounded-md border btn-cancel js-close-modal">Cancelar</button>
        <div class="ml-auto">
          <button type="submit" class="px-5 py-2 rounded-md text-white btn-green">Guardar Cliente</button>
        </div>
    </div>
  </form>
@endcomponent
