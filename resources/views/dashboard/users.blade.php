...
@section('content')
	@if(!empty($showCreate))
		@include('components.forms.formUser')
	@else
	<main class="p-6" style="font-family: var(--font-secondary);">
		<div class="flex items-start justify-between mb-6">
			<div>
				<h1 class="text-3xl font-extrabold mb-1" style="font-family: var(--font-primary); color: var(--text-title);">Usuarios</h1>
				<p class="text-sm text-gray-600">Gestiona la información de los usuarios</p>
			</div>

			<div class="ml-4">
	<a href="{{ route('dashboard.users.create') }}" data-role="admin" class="inline-flex items-center px-4 py-2 rounded-md text-white btn-green" data-nav style="background: var(--green);">
		...
		<div class="mb-6">
			...
			<div class="relative">
				...
				<input id="search" name="search" type="search" placeholder="Buscar por nombre del usuario" class="w-full pl-12 pr-4 py-3 rounded-lg border" style="background: var(--bg-input); border-color: var(--border);">
			</div>
		</div>

		<div>
			@include('components.lists.listUser')
		</div>
		</div>
	</main>
	@endif
