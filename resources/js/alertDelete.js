export default function confirmDelete(message = '¿Estás seguro?') {
  return new Promise((resolve) => {
    ...
    wrapper.innerHTML = `
      ...
      <div role="dialog" aria-modal="true" style="background:white;border-radius:8px;padding:18px;max-width:420px;width:90%;box-shadow:0 10px 30px rgba(0,0,0,0.2);z-index:10000;">
        <div style="font-weight:600;font-size:16px;margin-bottom:8px;color:#111">Confirmar eliminación</div>
        ...
        <div style="display:flex;gap:8px;justify-content:flex-end">
          ...
          <button id="confirm-delete-ok" style="padding:8px 12px;border-radius:6px;border:0;background:#dc2626;color:#fff;">Eliminar</button>
        </div>
      </div>
    `;
...
if (typeof window !== 'undefined') {
  window.confirmDelete = (msg) => confirmDelete(msg);
}
